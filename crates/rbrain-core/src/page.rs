...
impl Language {
    ...
    fn is_traditional(text: &str) -> bool {
        ...
        let oc_opt = OPENCC.get_or_init(|| OpenCC::from_config(BuiltinConfig::T2s).ok());
...
mod tests {
    ...
    fn test_language_detect() {
        // English
        assert_eq!(
            Language::detect("This is a simple English sentence."),
            Language::En
        );

        // Simplified Chinese
        assert_eq!(
            Language::detect("这是一篇关于人工智能的测试文档。"),
            Language::ZhHans
        );
        assert_eq!(
            Language::detect("支持向量机（SVM）是一种监督学习方法。"),
            Language::ZhHans
        );
        ...
        // Traditional Chinese
        assert_eq!(
            Language::detect("這是一篇關於人工智慧的測試文檔。"),
            Language::ZhHant
        );
        ...
        // Japanese
        assert_eq!(
            Language::detect("これは人工知能に関するテスト文書です。"),
            Language::Ja
        );
        ...
        // Korean
        assert_eq!(
            Language::detect("이것은 인공지능에 관한 테스트 문서입니다."),
            Language::Ko
        );
